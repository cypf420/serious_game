...
export const ACTION_COPY: Record<string, ActionCopy> = {
  ...
  public_hearing: { title: "公开听证", purpose: "围绕议题听取参与者意见并记录程序性结论。", when: "需要组织当前可参与对象进行公开讨论时。", how: "确定地点、议题和参与人员，核对当前表单要求后发起。", result: "形成公开听证记录和程序性结论。" },
...
export function sceneTutorial(scene: string): TutorialDefinition | null {
  ...
  return { id: `scene:${scene}`, revision: 2, title: copy.title, steps };
}
