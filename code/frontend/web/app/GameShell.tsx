          ...
          <div data-tutorial-id={tutorialContext.scene === "morning" ? "morning" : undefined} className="story-scroll" ref={storyScrollRef} aria-live="polite" data-scene-match={currentScene.matchedBy}>
            ...
            {(primaryScene === "narrative" || primaryScene === "conversation") && <section className={activeConversation ? "gal-stage conversation-mode" : decisionReady ? "gal-stage decision-mode" : "gal-stage"} data-primary-scene={primaryScene} data-testid={state.active_conversation ? "active-conversation-character" : undefined}>
              ...
              <div className={stageSpeaker ? "gal-dialogue has-speaker" : "gal-dialogue narration"}>
                ...
                })}</div>}</div> : conversationStreamingReply && !conversationStreamingReply.complete ? <p>{stageText}<i className="stream-cursor" aria-hidden="true" /></p> : <PagedReading key={`${api.accountId}:${sessionId}`} text={stageText} storageKey={`serious-game:story-reading:v1:${api.accountId}:${sessionId}:${currentLine?.id || "idle"}`} narrative onPreviousBoundary={narrative.currentIndex > 0 ? () => dispatchNarrative({ type: "PREVIOUS" }) : undefined} onNextBoundary={narrative.currentIndex < playerLines.length - 1 ? () => dispatchNarrative({ type: "NEXT" }) : undefined} controlsExtra={<button className="history-toggle" onClick={() => setShowHistory(value => !value)}>{showHistory ? "关闭回看" : "剧情回看"}</button>} /> }
                {(decisionReady || (conversationStreamingReply && !conversationStreamingReply.complete)) && <nav data-tutorial-id="narrative-controls" className="narrative-controls" aria-label="剧情阅读控制">
...
function SceneSummary({ state, commands, requiredOpportunity, governanceAction, decisionReady, onNavigate, onEndDay }: { state: Dict; commands: Dict; requiredOpportunity: Dict | null; governanceAction: Dict | null; decisionReady: boolean; onNavigate: (panel: PanelName) => void; onEndDay: () => void }) {
  ...
  const requiredOpportunityDescription = requiredOpportunityPending
    ? playerText(requiredOpportunity?.reason || requiredOpportunity?.description || requiredOpportunity?.entry_description, "请先寻找当前指定会谈，了解村庄关系与顾虑。")
    : "";
...
function OpportunityPanel({ data, signingOnly = false, activeConversation, onStart, onContinue, onOpenProfile }: { data: Dict | null; signingOnly?: boolean; activeConversation: Dict | null; onStart: (item: Dict) => void; onContinue: () => void; onOpenProfile: (character: Character) => void }) {
  ...
  return <div className="people-relationship-view">
    ...
    <div className="card-list people">{shown.length ? shown.map(person => {
      ...
      const actions = routines.filter(item => item.npc_id === person.npc_id && (!signingOnly || item.action_id === "household_visit"));
...
function ContractWorkspace({ onTutorialStageChange, contract, governance, state, busy, api, sessionId, onPerform, onOpenContract, onDirtyChange, onContinue }: { onTutorialStageChange: (value: { id: string; stage: import("./tutorial/types").ContractTutorialStage }) => void; contract: Dict; governance: Dict; state: Dict; busy: boolean; api: GameApi; sessionId: string; onPerform: (action: () => Promise<Dict>, success: string, aiLabel?: string) => Promise<void>; onOpenContract: (contract: Dict) => void; onDirtyChange: (dirty: boolean) => void; onContinue: () => void }) {
  ...
  return <div className="contract-workspace">
    ...
    <div className="contract-final-actions">{editable && contract.current_version > 0 && <div><p>对方接受后合同立即生效，并扣除约定款项和资源。</p>{submitBlockedReason && <p id="contract-submit-status" role="status">{submitBlockedReason}</p>}{operationError && editing && <p className="contract-submit-error">保存未成功：{playerText(operationError)}</p>}{editing && <button type="button" className="secondary" onClick={event => { event.currentTarget.closest(".contract-workspace")?.querySelector(".contract-terms-form")?.scrollIntoView({ block: "start", behavior: "smooth" }); }}>返回方案检查</button>}<button data-tutorial-id="contract-submit" aria-describedby={submitBlockedReason ? "contract-submit-status" : undefined} disabled={!showExistingReply && Boolean(submitBlockedReason)} onClick={showExistingReply ? event => { const feedback = event.currentTarget.closest(".contract-workspace")?.querySelector<HTMLElement>('[data-tutorial-id="contract-feedback"]'); feedback?.scrollIntoView({ block: "center", behavior: "instant" }); feedback?.focus({ preventScroll: true }); } : submitReview}>{showExistingReply ? "查看本次答复" : busy ? "正在处理…" : dirty || editing ? "请先保存方案" : "提交签约"}</button></div>}{status === "signed" && <div data-tutorial-id="contract-signed" className="signed-contract-seal"><b>已签署</b><span>第 {contract.signed_day} 日</span></div>}</div>
  </div>;
}
