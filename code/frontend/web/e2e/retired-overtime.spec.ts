...
async function fixture(page: Page, options: { auto?: boolean; readOnly?: boolean; hidden?: boolean; unavailable?: string; failWrite?: boolean; evidenceDecision?: boolean } = {}) {
  ...
  const state = { session_id: session, status: "active", state_version: 1, story: { day: 2 }, ledger: { action_points: { remaining: 0, daily_cap: 8, overtime_available: true, chapter_overtime_remaining: 3 }, budget: { available: 8000 } } };
  ...
  await page.route("**/api/backend/**", async route => {
    ...
    if (path === "/health/ready") body = { authentication_required: false, model_consent_required: false };
      ...
      else body = { state, commands: { can_end_day: true }, feed: { items: [{ id: "tutorial-line", content_instance_id: "decision-entry", presentation_phase: options.evidenceDecision ? "decision" : "narration", story_day: 2, kind: options.evidenceDecision ? "decision" : "narration", text: "请根据当前情况安排今天的工作。" }], cursor: 1 } };
    }
...
for (const width of [1419, 390]) test(`retired mechanics stay absent with exhausted energy at ${width}`, async ({ page }) => {
  await page.setViewportSize({ width, height: 898 });
  await fixture(page, { auto: false });
  await enter(page);
  await expect(page.getByRole('button', { name: '申请加班', exact: true })).toHaveCount(0);
  await expect(page.getByText('身心状态', { exact: true })).toHaveCount(0);
  await expect(page.locator('[data-tutorial-id="end-day"]')).toBeVisible();
