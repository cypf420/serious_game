...
test("signed contract updates fiscal balance even when view refresh fails", async ({ page }) => {
  ...
  let contract = { contract_id: "c1", batch_id: "b1", household_id: "YUAN-01", signatory_npc_id: "npc_yuan_guilan", signatory_name: "袁桂兰", status: "rejected", current_version: 2, review_version: 2, review_reason: "请确认扶手。", audit_status: "not_required", contract_text: "搬离日：第83日\n交房日：第83日\n住房已核实设有扶手。", can_review: false, conversation_available: true, term_sheet: { cash_amount: 27, move_out_day: 83, housing_delivery_day: 83, transition_months: 12 } };
  await page.route("**/api/backend/**", async route => {
    ...
    if (endpoint === "/health/ready") body = { authentication_required: false, model_consent_required: false };
      ...
      expect(submitted.auto_arrange).toBe(true);
      for (const field of ["budget_envelope", "transition_months", "move_out_day", "housing_delivery_day"]) expect(submitted).not.toHaveProperty(field);
      if (rejectSave) {
        await route.fulfill({ status: 409, contentType: "application/json", body: JSON.stringify({ error: { code: "RESOURCE_INSUFFICIENT", message: "测试方案条件不符，请检查住房安排。" } }) });
        return;
      }
      contract = { ...contract, status: "draft", can_review: true, current_version: 3, term_sheet: { ...contract.term_sheet, cash_amount: 28 }, contract_text: "现金补偿：28万元\n搬离日：第83日\n交房日：第83日" };
  ...
  await page.goto("/");
  await expect(page.locator(".online")).toContainText("游戏已就绪");
  ...
  await dialog.getByRole("button", { name: "查看本次答复", exact: true }).click();
  await expect(dialog.locator('[data-tutorial-id="contract-feedback"]')).toBeFocused();
  expect(writes).toHaveLength(0);
  await dialog.getByRole("button", { name: "修改方案", exact: true }).click();
  await dialog.locator('[name="cash_amount"]').fill("28");
  await expect(dialog.getByRole("button", { name: "请先保存方案", exact: true })).toBeDisabled();
  await expect(dialog.locator("#contract-submit-status")).toContainText("尚未保存");
  ...
  await expect(dialog.locator(".contract-submit-error")).toContainText("测试方案条件不符");
  await expect(dialog.locator('[name="cash_amount"]')).toHaveValue("28");
  await expect(dialog.getByRole("button", { name: "请先保存方案", exact: true })).toBeDisabled();
  expect(writes.filter(path => path.endsWith('/review'))).toHaveLength(0);
  rejectSave = false;
  await dialog.getByRole("button", { name: "保存方案并预览合同", exact: true }).click();
  ...
  await expect(dialog.locator('[data-tutorial-id="contract-submit"]')).toHaveText("正在处理…");
